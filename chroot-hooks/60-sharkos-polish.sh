...
# =============================================================================
echo "[1/5] Installation shark-doctor..."
...
case "$KERNEL" in
  *liquorix*) OK_GOOD "Kernel Liquorix détecté ($KERNEL)" ;;
  *) OK_WARN  "Kernel standard ($KERNEL) — Liquorix non actif" ;;
esac
...
# =============================================================================
# 2. shark-firewall — switch entre profils UFW
# =============================================================================
echo "[2/5] Installation shark-firewall..."
...
# =============================================================================
# 3. shark-clip — clipboard chiffré
# =============================================================================
echo "[3/5] Installation shark-clip..."
...
# =============================================================================
# 4. shark-restore — restauration configs depuis /etc/sharkos/backups
# =============================================================================
echo "[4/5] Installation shark-restore..."
...
# =============================================================================
# 5. shark-arc — archiveur intelligent
# =============================================================================
echo "[5/5] Installation shark-arc..."
...
# =============================================================================
# 7. INSTALLER + BUNDLE CALAMARES (kit d'installation sur disque)
# =============================================================================
echo "[7] Installer SharkOS + bundle Calamares…"
...
#!/usr/bin/env bash
# Shark-Installer (stub redistribué dans l'ISO)
# Source complète : scripts/sharkos-installer du repo Git.
# Stub minimal pour que sharkos-welcome ne tombe pas en erreur.
PURPLE="\033[1;35m"; RESET="\033[0m"
...
# Calamares config (utilisée si Calamares binaire est installé dans la session live)
...
# =============================================================================
# 8. CALAMARES UI POLISH (overwrite prior minimal branding + ship verify-iso)
# =============================================================================
echo "[8] Calamares UI polish (QSS Dracula + intro/sidebar HTML + multi-size logos)…"

# Re-ship the rich QSS (overrides the earlier minimal QSS that hook 60 wrote)
for SRC in \
  ...
  FNAME="$(basename "$SRC")"
  if [[ -f "$ROOT/../../$SRC" ]]; then
    install -m 0644 "$ROOT/../../$SRC" \
      /etc/calamares/sharkos/branding/"$FNAME" || true
  fi
done
...
if [[ -f "$ROOT/../../scripts/03-verify-iso.sh" ]]; then
  install -m 0755 "$ROOT/../../scripts/03-verify-iso.sh" \
    /usr/local/bin/sharkos-verify-iso
fi

echo "   ✅ Calamares Dracula polish appliqué (QSS, intro, sidebar, splash, multi-l"
