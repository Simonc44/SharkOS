...
# PAM — autoriser l'autologin sans mot de passe pour ce groupe
mkdir -p /etc/pam.d
cat > /etc/pam.d/lightdm-autologin << 'EOF'
# SharkOS — autorise autologin pour les membres du groupe `autologin`
auth sufficient pam_succeed_if.so user ingroup autologin
auth required pam_deny.so

account sufficient pam_succeed_if.so user ingroup autologin
account required pam_deny.so

password sufficient pam_succeed_if.so user ingroup autologin
password required pam_deny.so

session sufficient pam_succeed_if.so user ingroup autologin
...
# =============================================================================
# 4. GRUB THEME — fond Shark Dragon + menu Dracula
# =============================================================================
echo "[4/8] GRUB theme Shark Dragon..."

mkdir -p /usr/share/grub/themes/sharkdragon

cat > /usr/share/grub/themes/sharkdragon/theme.txt << 'EOF'
title-text: "🦈 SharkOS Dragon Edition"
title-color: "#e94560"
title-font: "JetBrains Mono Regular 18"

desktop-image: "background.png"
desktop-color: "#0d0221"
...
+ boot menu {
  ...
  width = 60%
  height = 50%
  item font = "JetBrains Mono Regular 14"
  item color = "#eefaf8"
  item selected color = "#e94560"
  item-height = 36
  item-padding = 12
  item-spacing = 6
}
...
+ label {
  text = "🦈 SharkOS 2.0 Dragon — login : shark / shark"
  color = "#bd93f9"
...
# Fond GRUB — dégradé violet foncé + silhouette requin en typo
if command -v convert &>/dev/null; then
  convert -size 1920x1080 gradient:'#0d0221-#1a0a2e' \
    -fill 'rgba(233,69,96,0.18)' -gravity center \
    -font DejaVu-Sans-Bold -pointsize 240 -annotate +0+0 '🦈' \
...
show_menu() {
  ...
  printf "  ${CYAN}1)${RESET} 🛠  Installer SharkOS sur le disque (Calamares)\n"
  ...
  case "$CHOICE" in
    1)
      ...
      sudo calamares 2>/dev/null || \
      sudo bash /usr/local/bin/sharkos-installer 2>/dev/null || \
      printf "  ${RED}⚠ calamares indisponible — installe-le via apt ou utilise Refracta.${RESET}\n"
      ;;
    2)
...
QUOTES=(
  ...
  "🦈 Reste sleek : drivers GPU, kernel Liquorix, ZRAM 50%, BBR TCP."
